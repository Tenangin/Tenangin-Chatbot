...
def retrieve(state: State):
    ...
    recent_context = " ".join(history[:4])  # ambil 4 terakhir (terbaru di awal)
...
def generate(state: State):
    ...
    # Masukkan Q & A terbaru ke depan
    new_history.insert(0, f"AI: {response.content}")
    new_history.insert(0, f"User: {state['question']}")

    # Potong history supaya maksimal 3 pasang (6 item)
    MAX_HISTORY_PAIRS = 3
